...
namespace ROAD {
	...
	RoadMapPath * Graph::getPath( const BaseAgent * agent, const Goal * goal ) {
	...
	RoadMapPath * Graph::getPath( size_t startID, size_t endID ) {
		...
		AStarMinHeapPlus heap( _HEAP + threadNum * N,
						   _DATA + threadNum * DATA_SIZE,
						   _STATE + threadNum * STATE_SIZE,
						   _PATH + threadNum * N, N );//路径规划用到了A* algorithm
	#else
		AStarMinHeapPlus heap( _HEAP, _DATA, _STATE, _PATH, N );
	#endif
		...
		heap.push( (unsigned int)startID );//把起点放进heap

		//下面是找路径的过程
		bool found = false;
		while ( !heap.empty() ) {
			unsigned int x = heap.pop(); //第一次执行的时候，将起点startID赋给x
			...
			GraphVertex & vert = _vertices[ x ]; //用点的id找到点的位置

			const size_t E_COUNT = vert.getEdgeCount();//返回与这个点相连的边数量
			for ( size_t n = 0; n < E_COUNT; ++n ) { //遍历每一个边，第n个边
				const GraphVertex * nbr = vert.getNeighbor( n ); //返回第n个边相连的那个点
				size_t y = nbr->getID();//得到那个点的id，找到了此条边的另一个点的id
				if ( heap.isVisited( (unsigned int)y ) ) continue; //如果这个点已经被访问过，不走回头路，所以跳过这条边
				float distance = vert.getDistance( n );//计算这两个点之间的直线距离，即边的长度
				float tempG = heap.g( x ) + distance;//？？？？
				
				bool inHeap = heap.isInHeap( (unsigned int)y );//查看端点是否在heap里
				if ( ! inHeap ) {
					heap.h( (unsigned int)y, computeH( y, goalPos ) );//先
				}
		...
		//上面已经找到了路径，放在heap里，下面只是一个一个弹出路径组装成path的过程
