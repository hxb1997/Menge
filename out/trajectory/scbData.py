...
def Location_estimate(x,y,locations):
    shop_index = 0
    for i in range(1, len(locations)):
        if(x<= locations[i-1][0]+2 and x>= locations[i-1][0]-2 and y<= locations[i-1][1]+2 and y>= locations[i-1][1]-2):
            shop_index = i
    if(x<= locations[i][0]+1 and x>= locations[i][0]-1 and y<= locations[i][1]+2 and y>= locations[i][1]-2):
        shop_index = i + 1
    return shop_index
#shop_index: 0 walk    1 2 3 4 shop   5 out


def process(data, locations, shop_num):
    route_agent = np.zeros([data.agentCount(), data.totalFrames(), 5]) #x,y,shop_index, buy or not buy ,state
    shop_access = np.zeros(shop_num) # (0 1 2 3 buy times)
    for frame in range(0, data.totalFrames()):
        data_frame = data.next()
        for agent in range(0, data.agentCount()):
            ...
            route_agent[agent][frame][2] = Location_estimate(data_frame[0][agent][0], data_frame[0][agent][1], locations)
            # find the state of buy, and  the shop buy number + 1
            if (data_frame[0][agent][3] % 3 == 1 and data_frame[0][agent][3] <= 53 and route_agent[agent][frame-1][4] % 3 != 1):
                route_agent[agent][frame][3] = 1
                shop_access[int(route_agent[agent][frame][2])-1] = shop_access[int(route_agent[agent][frame][2])-1] + 1 # buy times
            route_agent[agent][frame][4] = data_frame[0][agent][3]


    return route_agent, shop_access
...
def main():
    ...
    #route_agent, shop_access = process(data, locations, shop_num)
    #print  route_agent[0, :, 2]
    #np.set_printoptions(threshold='nan')
    #print "\tThe info of an agent:"
   # print route_agent[122]
    print "\tShop and buy times:"
    #print shop_access
   # print data.next(50)[0][:, 3]
    print data.next()
