        ...
        if (isinstance(i, np.ndarray)):
            if (i.dtype == np.bool):
                if (i.size <= self.agtCount):
                    ...
                    raise ValueError, "Selection boolean array is too large - given %d elements for %d agents" % (
                    i.size, self.agtCount)
            else:
...
class FrameSet:
    ...
    def writeFrame(self, frame, file, agent=-1):
        ...
        file.write(frame.toBinary(agent))
...
class NPFrameSet(FrameSet):
    ...
    def prev(self, stride=1):
        ...
        if (self.currFrameIndex >= stride):
            ...
            if (self.currFrame is None):
                self.currFrame = np.empty((self.readAgtCount, self.colCount), dtype=np.float32)
            self.currFrame[:, :] = np.reshape(
                np.fromstring(self.file.read(self.frameSize), np.float32, self.readAgtCount * self.colCount),
                (self.readAgtCount, self.colCount))
            # seek forward based on skipping
            self.file.seek(self.readDelta, 1)  # advance to next frame
            self.file.seek(self.strideDelta, 1)  # advance according to stride
...
class SCBDataMemory:
    ...
    def setData(self, data, version, timeStep, ids=None):
        ...
        if (not self.ids is None):
            if (len(self.ids) != data.shape[0]):
                raise AttributeError, "The agent class id list doesn't match the number of agents in the data.  Data has %d, ids has %d" % (
                len(self.ids), data.shape[0])
    ...
    def setNext(self, frameIdx):
        ...
        if (self.nextFrame < 0 or frameIdx > self.data.shape[2]):
            raise ValueError, 'The index %d is outside of the feasible range: [%d, %d]' % (
            -self.data.shape[2], self.data.shape[2])
...
def testNP():
    ...
    for i in range(fullData.shape[2]):
        ...
        for a in range(f1.readAgtCount):
            error += np.sum(
                np.array([frame.agents[a].pos.x - fullData[a, 0, i], frame.agents[a].pos.y - fullData[a, 1, i]]) ** 2)
        frame, idx = f1.next()
        if (error > 1e-5):
...
def Search(array,t):
    ...
    for i in range(0, height):
        if(t > array[i] and t < array[i+1]):
            return i+1

def Location_estimate(x, y): # 120,-120 5,0  [4,5]
    x_array = [-69, -27, 15, 57, 99]
    y_array = [-87.5, -48.5, -9.5, 29.5]
    shop_matrix = np.arange(30).reshape(5, 6)
    x_index, y_index = Search(x_array, x), Search(y_array, y)
    shop_index = shop_matrix[4-y_index, x_index]
    return shop_index



def process(data, shop_num):
    route_agent = np.zeros([data.agentCount(), data.totalFrames(), 4])  # x,y, state, shop_index
    shop_access = np.zeros(shop_num)  # (index=0-29 value=shopping_times)
    profit_record = np.zeros([int((data.totalFrames() - 1) / 10)+1, 30])  # every moment, 30 shop profit
    profit_shop = np.ones(30)
    shopping_frame_record = []
    Store_Max_loadrate = 1

    for i in range(0, data.agentCount()):
        shopping_frame_record.append([0])
    for frame in range(0, data.totalFrames()):
        data_frame = data.next()  # data_frame[0]:num_agents*4-----[x,y,orient,state]
        for agent in range(0, data.agentCount()):
            route_agent[agent][frame][0] = data_frame[0][agent][0]
            route_agent[agent][frame][1] = data_frame[0][agent][1]
            route_agent[agent][frame][2] = data_frame[0][agent][3]
            # below: not first few frame ,so frame-1>0; state changes; not toexit or stop; pre not state of leave
            #
            if (frame >= 3 and route_agent[agent][frame][2] != route_agent[agent][frame-1][2] and
                    route_agent[agent][frame][2] <= 150 and route_agent[agent][frame-1][2] % 5 != 0and route_agent[agent][frame-1][2] % 5 != 4):
                route_agent[agent][frame][3] = Location_estimate(route_agent[agent][frame-1][0], route_agent[agent][frame-1][1])
                shop_access[int(route_agent[agent][frame][3])] = shop_access[int(route_agent[agent][frame][3])] + 1
                profit_record[int((frame - 1)/10)][int(route_agent[agent][frame][3])] = profit_record[int((frame - 1)/10)][int(route_agent[agent][frame][3])]+ profit_shop[int(route_agent[agent][frame][3])]
                shopping_frame_record[agent].append(int(frame))


    #print shopping_frame_record
    #print profit_record


    return route_agent, shop_access, profit_record, shopping_frame_record


# return x,y,index_shop
'''
def Show(shop_access):
    shop_eat = [1, 28, 24, 4, 12, 6, 15, 19, 18, 29]  # green
    shop_play = [0, 11, 3, 27, 14, 25, 5, 16, 8, 21]  # yellow
    shop_buy = [9, 20, 7, 22, 10, 23, 17, 2, 26, 13]  # blue
    for i in range(0, 10):
        if i == 0:
            plt.bar(shop_eat[i], shop_access[shop_eat[i]], 0.3, color="green", label='Eat')
            plt.bar(shop_play[i], shop_access[shop_play[i]], 0.3, color="yellow", label='Play')
            plt.bar(shop_buy[i], shop_access[shop_buy[i]], 0.3, color="blue", label='Buy')
        else:
            plt.bar(shop_eat[i], shop_access[shop_eat[i]], 0.3, color="green")
            plt.bar(shop_play[i], shop_access[shop_play[i]], 0.3, color="yellow")
            plt.bar(shop_buy[i], shop_access[shop_buy[i]], 0.3, color="blue")

    # plt.bar(np.arange(30), shop_access, 0.4, color="green")
    plt.xlabel("Index of shop")
    plt.ylabel("Number of shopper")
    plt.title("Result")
    for x, y in zip(np.arange(30), shop_access):
        plt.text(x + 0.05, y + 0.05, '%d' % y, ha='center', va='bottom')
    plt.legend()
    plt.show()
'''

def location_3(shop_index,array,flag):
    #flag = 0signin 1arrive 2leave 3out
    #(-20, 10)  (3, 15)  (-13, 38)
    ...
    if flag == 0:
        ...
        if shop_index in shop_eat:
            array.append('-20 0 10')
        elif shop_index in shop_play:
            array.append('3 0 15')
        elif shop_index in shop_buy:
            array.append('-13 0 38')
    elif flag == 3:
        array.append('-20 0 -5')

def location_3_1(shop_index, profit_record_3shop, frame,num):
    shop_eat = [1, 28, 24, 4, 12, 6, 15, 19, 18, 29] # the shop number 0-9
    shop_play = [0, 11, 3, 27, 14, 25, 5, 16, 8, 21]
    shop_buy = [9, 20, 7, 22, 10, 23, 17, 2, 26, 13]
    if shop_index in shop_eat:
        profit_record_3shop[frame][0] = profit_record_3shop[frame][0] + num
    elif shop_index in shop_play:
        profit_record_3shop[frame][1] = profit_record_3shop[frame][1] + num
    elif shop_index in shop_buy:
        profit_record_3shop[frame][2] = profit_record_3shop[frame][2] + num

    return profit_record_3shop

def Out_to_Unity(route_agent, data, profit_record,shopping_frame_record):
    agent_info = []
    profit_info = []
    #Profit_record = np.zeros([int(data.totalFrames()-1)/10, 30]) #every moment, 30 shop profit
    #Profit_shop = np.ones(30)
    #for agent in range(0, data.agentCount()):
    profit_record_3shop = np.zeros([len(profit_record), 3])
    '''  TXT profit '''
    for frame in range(0, len(profit_record)):
        for shop in range(0, 30):
            if profit_record[frame][shop] != 0:
                profit_record_3shop = location_3_1(shop, profit_record_3shop, frame, profit_record[frame][shop])
        profit_info.append(str(int(profit_record_3shop[frame][0])))
        profit_info.append(' ')
        profit_info.append(str(int(profit_record_3shop[frame][1])))
        profit_info.append(' ')
        profit_info.append(str(int(profit_record_3shop[frame][2])))
        profit_info.append('\n')

    #TXT_INFO.append(str(profit_record))
    #print TXT_INFO
    #print profit_record
    '''  TXT route '''
    for agent in range(0, 300):
        #p = 0
        #for moment in agent_shopping_moment[agent][0]:
          #  Profit_record[moment][agent_shopping_moment[agent][1][p]] = Profit_record[moment][agent_shopping_moment[agent][1][p]] + Profit_shop[agent_shopping_moment[agent][1][p]]
         #   p = p + 1
        i = 0
        for frame in shopping_frame_record[agent]:
            if(i == 0):
                agent_info.append('[signin] ')
                agent_info.append(str(agent))
                agent_info.append(' ')
                agent_info.append('0 ')
                location_3(route_agent[agent][frame][3], agent_info, 0)
                agent_info.append('\n')
                i = 1
            elif i == 1 and int(frame/10) <= 48:
                agent_info.append('[arrive] ')
                agent_info.append(str(agent))
                agent_info.append(' ')
                agent_info.append(str(int((frame-1)/10)))
                agent_info.append(' ')
                location_3(route_agent[agent][frame][3],agent_info,1)
                agent_info.append('\n')
                agent_info.append('[leave] ')
                agent_info.append(str(agent))
                agent_info.append(' ')
                agent_info.append(str(int((frame-1 +20) / 10)))
                agent_info.append(' ')
                location_3(route_agent[agent][frame][3], agent_info, 2)
                agent_info.append('\n')
            if frame == shopping_frame_record[agent][-1] and i == 1:
                agent_info.append('[signout] ')
                agent_info.append(str(agent))
                agent_info.append(' ')
                agent_info.append(str(int((frame-1+50)/10)))
                agent_info.append(' ')
                location_3(route_agent[agent][frame][3], agent_info, 3)


        if agent != 299 or frame != shopping_frame_record[agent][-1]:
            agent_info.append('\n')
            agent_info.append('\n')
    ...
    #p2u = Py2Unity()
    #print("connect ok")
    #p2u.SendToUnity(''.join(TXT_INFO))  # "save_ok"
    #print("send over")
    #print TXT_INFO
    #p2u.RecFromUnity()
    f_agent = open('./town_all/TEST/agent.txt', 'w')
    f_agent.write(''.join(agent_info))
    f_agent.close()
    f_profit = open('./town_all/TEST/profit.txt', 'w')
    f_profit.write(''.join(profit_info))
    f_profit.close()
    print ("TXT generate over")

def ScbProcess():
    '''A simple script for summarizing the script'''
    shop_num = 30
    data = NPFrameSet('.\out.scb')
    ...
    route_agent, shop_access, profit_record, shopping_frame_record = process(data, shop_num)
    #print shopping_frame_record
    #print profit_record
    Out_to_Unity(route_agent, data, profit_record, shopping_frame_record)
    # print  route_agent[0, :, 2]

    #print agent_shopping_moment
    # print "\tThe info of an agent:"
    #print route_agent[90]
    #print "\tShop times:"
    #print shop_access
    #print "\tShopping sum:"
    #print sum(shop_access)
    #shop_play = shop_access[0]+shop_access[3]+shop_access[5]+shop_access[8]+shop_access[11]+shop_access[14]+shop_access[16]+shop_access[21]+shop_access[25]+shop_access[27]
    #shop_eat = shop_access[1]+shop_access[4]+shop_access[6]+shop_access[12]+shop_access[15]+shop_access[18]+shop_access[19]+shop_access[24]+shop_access[28]+shop_access[29]
    #shop_buy = sum(shop_access) - shop_eat - shop_play
    #print "\tShopping play:"
    #print shop_play/sum(shop_access)
    #print "\tShopping eat:"
    #print shop_eat / sum(shop_access)
    #print "\tShopping buy:"
    #print shop_buy / sum(shop_access)
    #print data.next(300)[0]
    # print data.next()
    #Show(shop_access)
    #print shop_access
    return shop_access

##out:[x,y,orient,state]
